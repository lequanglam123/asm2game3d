...
public class FightingController : MonoBehaviour
{
    ...
    void PerformAttack(int attackIndex)
    {
        if (Time.time - lastAttackTime > attckCooldown)
        {
            ...
            // vong qua tung doi thu
            foreach(Transform opponent in opponents) 
            { 
                if(Vector3.Distance(transform.position, opponent.position) <= attackRadius) 
                {
                    opponent.GetComponent<OpponetAI>().StartCoroutine(opponent.GetComponent<OpponetAI>().PlayHitDamageAnimation(attackDamages));
                }
            }
        ...
        {
            // Neu nguoi choi thuc hien mot cuoc tan cong qua nhanh hay thong bao cho ho
            Debug.Log("Cannot perfrom attack yet. Cooldown time remaning.");
        }
    }
